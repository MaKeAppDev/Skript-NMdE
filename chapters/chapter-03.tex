...
\begin{equation}
\begin{split}
y(0) &= 0,\ y(t) = \int\limits_0^t y(\tau) \diff \tau\\
y(t+ \Delta t) &= \int\limits_0^t y(\tau) \diff \tau + \int\limits_t^{t+\Delta t} y(\tau) \diff\tau = y(t) + \int\limits_t^{t+\Delta t} y(\tau) \diff\tau\\
...
\item Genauigkeit: $\epsilon^{(A)}(\nu \cdot \Delta t) = \hat{x}(\nu) - x(\nu) \approx (\Delta t)^k$ (für$|p_\infty \Delta t| << 1$)
